...
- 本分支要求的 5 张 Image2 场景背景与接触表已生成并验收，详见 `assets/concepts/artist_thread_image2_scene_round_20260620/ARTIST_THREAD_IMAGE2_SCENE_DELIVERY.md`。
...
4. 将已验收的 5 张场景 PNG 按 `HUMAN_ARTIST_SCENE_BRIEF.md` 的文件名接入卡牌专用场景，并先验证底部 UI 覆盖区与角色站位。
...
- 场景背景已就位，但尚未被卡牌运行时引用。接线时不得改变底部静区和中部角色站位，否则会重新引入 UI 覆盖与可读性风险。
