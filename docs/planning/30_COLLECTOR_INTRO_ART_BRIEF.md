...
当前程序侧已可用占位 UI 表现这段流程，但缺少收藏家正式美术。画师优先在 `feature/art-iteration` 分支交付本文件列出的资产。
...
```text
assets/card_demo/actors/collector/collector_bust_idle_0.png
assets/card_demo/actors/collector/collector_bust_idle_1.png
assets/card_demo/actors/collector/collector_bust_idle_2.png
assets/card_demo/actors/collector/collector_bust_idle_3.png
assets/card_demo/actors/collector/collector_bust_speak_0.png
...
assets/card_demo/actors/collector/collector_bust_speak_5.png
```

规格：

- 单帧 `768x768`，透明 PNG。
- 构图为半身像或上半身，面向右前或正前偏右。
- 建议 draw size：`320x320` 到 `420x420`。
- `idle` 4 帧，6 FPS：衣袍、记录纸、头部极轻微动。
...
```text
assets/card_demo/actors/collector/collector_bust_death_comment_0.png
...
```
...
```json
{
  ...
  "frame_count": 4,
  "fps": 6,
  "anchor": {"mode": "center", "x": 384, "y": 384},
  "draw_size": [360, 360],
  "paths": [
    "assets/card_demo/actors/collector/collector_bust_idle_0.png"
  ],
...
P0：
1. 收藏家半身像透明 PNG 序列：idle 4 帧、speak 6 帧，单帧 768x768。
2. 无声圣匣开场背景：1920x1080。
