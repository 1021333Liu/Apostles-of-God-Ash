...
- 负责人：刘秉昂 / Codex
- 改动：
  - 将日志背包从 `Node2D` 绘制层改为 HUD 上层 `Control` 覆盖层，打开后完整盖住底部提示和右侧信息框，减少界面叠化。
  - 将 HUD 中的“神之胃囊”改为更可理解的“器官残骸：吞食胃”。
  - 将日志详情里的“胃囊反应”改为“器官判读”，并在空日志状态解释它用于显示器官如何解读记忆碎片。
  - 将日志类型从英文 rarity 转为中文：普通碎片、关键碎片、稀有碎片、Boss记忆。
  - 补充 `project.godot` 的 OpenGL 兼容渲染设置，减少试玩启动时的渲染后端问题。
- 原因：
  - 试玩反馈指出日志 UI 和已有 HUD 重叠严重，并且核心术语缺乏即时解释，玩家不清楚“神之胃囊/胃囊反应”是什么。
- 影响范围：
  - 日志背包 UI 层级、HUD 术语、日志详情文案、试玩启动兼容性。
- 验证：
  - 已用 Godot 4.6.3 headless 启动 `res://scenes/main.tscn`，无解析错误输出。
- 后续：
  - 下一轮应继续把 HUD 面板数量减少，保留战斗必需信息，把叙事阅读集中到 `P` 日志。
...
- 负责人：刘秉昂 / Codex
- 改动：
  - 为日志系统新增 `P / Tab` 打开与关闭的“圣匣日志背包”覆盖层。
  - 打开日志背包时暂停战斗输入与敌人推进，避免玩家读档案时被打死。
  - 支持 `A/D` 或方向键左右切换已归档碎片。
  - 日志背包显示碎片列表、当前碎片来源、正文、胃囊反应和“谷仓王故事”拼图进度。
  - HUD 操作提示补充 `P 日志`，让玩家知道日志系统是可交互系统，不只是后台计数。
- 原因：
  - 试玩反馈指出当前日志系统只有后台归档和圣匣摘要，缺少明确玩家交互，不能形成“日志背包/碎片化叙事”的系统感。
- 影响范围：
  - 输入映射、战斗暂停逻辑、日志 UI 覆盖层、HUD 操作提示。
- 验证：
  - 已用 Godot 4.6.3 headless 启动 `res://scenes/main.tscn`，无解析错误输出。
- 后续：
  - 下一轮可以把当前绘制型覆盖层拆成正式 `Control` UI，并加入已读/新获得标记与鼠标/手柄导航。
...
- 负责人：刘秉昂 / Codex
- 改动：
  - 新增 `data/log_fragments/low_whispering_field.json`，把低语田野首批日志碎片、胃囊反应和“谷仓王为什么献出胃”的第一故事改为数据文件。
  - 新增 `scripts/systems/log_fragment_database.gd` 和 `scripts/systems/log_archive_runtime.gd`，负责读取日志数据、按敌人来源归档碎片、记录本轮获得内容和计算主线拼图进度。
  - 新增 `scripts/ui/log_archive_panel.gd`，预留后续圣匣日志 UI 的文本组装入口。
  - 更新 `scripts/main.gd`，让击杀敌人自动归档日志碎片，战斗中显示短采样反馈，圣匣中显示碎片数量、本轮收集、最新胃囊反应和第一故事解锁文本。
  - 扩大圣匣归档文本区域，让故事解锁后可在当前原型里直接阅读。
- 原因：
  - 日志碎片是战斗和世界观之间的关键系统，需要从“击杀飘字”升级为可积累、可回看、可拼故事的核心循环。
- 影响范围：
  - 击杀反馈、圣匣档案显示、低语田野日志数据、第一故事解锁进度、后续文案交付格式。
- 验证：
  - 已用 Godot 4.6.3 headless 启动 `res://scenes/main.tscn`，无解析错误输出。
- 后续：
  - 下一轮可以把“自动归档”扩展为场上可拾取 `Area2D`，并做真正的圣匣日志列表/当前条目/故事进度三栏 UI。
