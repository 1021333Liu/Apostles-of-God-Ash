...
### 商业化优化：强化人物可读性、攻击反馈与系统 UI 美术约束

- 负责人：刘秉昂 / Codex
- 改动：
  - 优化玩家贴图绘制：增加地面阴影、蓝金轮廓、攻击光弧和常驻胃囊标记，让角色在麦田背景上更容易被看清。
  - 优化攻击反馈：命中后生成短促爆点，挥空时出现轻反馈，三段攻击的斩击颜色和亮度更明确。
  - 优化敌人与 Boss 反馈：贴图敌人受击时补充闪光外圈；谷仓王“胃囊暴露”时增加脉冲准星和瞄准线，让弱点窗口更像一个可执行战斗仪式。
  - 优化日志/圣匣 UI 图像：系统图片统一裁进安全区域，TextureRect 使用等比居中，Node2D 绘制的概念图也通过 `_fit_texture_rect()` 保持原图比例，避免图片超出屏幕或被拉伸变形。
  - 调整日志页布局：把正文区和右侧美术预览区分离，右侧预览从小图标升级为独立画面框。
  - 新增 `tools/commercial_slice_audit.ps1`，检查 UI 图片裁切、等比绘制、攻击反馈、Boss 弱点准星、概念图 `.import` 侧车文件和 Godot headless 启动。
  - 新增 `docs/planning/24_COMMERCIAL_POLISH_AUTOMATION.md`，记录主线程 + 子 Agent 只读审查 + 本地自动化脚本的商业化优化工作流。
- 原因：
  - 试玩反馈指出人物图不够优化、系统 UI 观感不商业、攻击和机制反馈仍偏原型，且日志/系统界面存在图片尺寸压迫和溢出风险。
- 影响范围：
  - `scripts/main.gd` 的角色/敌人/Boss 绘制、攻击反馈、日志页布局、圣匣概念图绘制；新增商业化审查脚本和协作流程文档；补齐新概念 PNG 的 Godot `.import` 文件。
- 验证：
  - `git diff --check` 通过。
  - `powershell -ExecutionPolicy Bypass -File .\tools\commercial_slice_audit.ps1` 通过，Warnings 0，Errors 0。
  - Godot 4.6.3 headless 启动 `res://scenes/main.tscn` 通过。
- 后续：
  - 仍需要正式透明 PNG / spritesheet、攻击音效、命中特效粒子和日志页正式 UI 皮肤，才能继续逼近商业成片质量。

## 2026-06-10

### 美术接入：把 2026-06-10 清爽彩色概念图放进可玩版

- 负责人：刘秉昂 / Codex
- 改动：
  - 将普通战斗房间背景切换为 `art_clean_color_wheat_room_20260610.png`，让试玩直接看到新的清爽彩色麦田方向。
  - 在战斗背景上增加轻量暗化与竞技场边界线，保证角色、敌人、危险区和 HUD 仍然可读。
  - 在圣匣待机画面叠入圣匣 UI、玩家和日志碎片概念图，作为游戏内美术方向展示。
  - 在 `P` 日志页加入圣匣 UI 背板和右侧概念预览，并根据碎片来源切换敌人 / Boss / 日志碎片预览图。
  - 扩展 `scripts/systems/art_asset_registry.gd`，集中加载本轮概念图，避免主场景硬编码资源路径。
- 原因：
  - 用户要求“放到游戏里”，本轮先作为可运行视觉基准接入，而不是等待正式透明帧全部切好。
- 影响范围：
  - 战斗房间背景、圣匣待机画面、日志页视觉展示和美术资源注册；未改战斗数值、日志掉落逻辑或关卡流程。
- 验证：
  - 已用 Godot 4.6.3 headless 启动 `res://scenes/main.tscn`，无脚本解析错误。
- 后续：
  - 需要把玩家、敌人、Boss 概念图继续切成透明 PNG 帧或 spritesheet，才能正式替换当前战斗精灵。

### 美术概念：拉取 2026-06-10 两轮概念图

- 负责人：美术 Agent / Codex
- 改动：
  - 从 `feature/art-iteration` 拉取 `assets/concepts/art_iteration_round_20260610/`，包含玩家、空腹者、饥民农夫、稻草人、谷仓王和日志碎片的 P0 可读性概念图。
  - 从 `feature/art-iteration` 拉取 `assets/concepts/art_clean_color_round_20260610/`，包含更清爽彩色线稿方向的玩家、敌人、麦田房间、谷仓王、日志碎片和圣匣 UI 概念图。
  - 新增 `docs/planning/ART_ROUND_20260610.md` 和 `docs/planning/ART_ROUND_CLEAN_COLOR_20260610.md`，记录本轮美术方向、文件清单和后续清稿要求。
- 原因：
  - 当前可玩版需要进入更明确的美术方向筛选阶段，先把概念参考纳入主开发分支，供后续 image2 / 人工清稿 / 运行帧切图使用。
- 影响范围：
  - 仅影响概念图资产和美术规划文档；没有替换 Godot 运行资产，也没有修改战斗、UI 或日志系统代码。
- 验证：
  - 已确认 12 张 PNG 概念图和 2 份美术说明文档进入当前分支工作区。
- 后续：
  - 选择麦田房间、简化 UI、日志碎片和角色轮廓中最可读的方案，清理成透明 PNG 运行帧后再接入 `art_asset_registry`。

## 2026-06-06

### 核心战斗：强化命中反馈与日志掉落节奏

- 负责人：核心战斗执行 Agent / Codex
- 改动：
  - 在 `scripts/main.gd` 增加轻量 hit stop、screen shake、受击红色闪屏和更长的玩家受击动画窗口。
  - 增强敌人受击闪白、三段斩击绘制强度和 Boss 弱点暴露反馈。
  - 将 Boss 弱点窗口调整为约 1.9 秒暴露、2.8 秒关闭，弱点额外伤害提高到 +24。
  - 在 `scripts/systems/log_archive_runtime.gd` 为日志碎片归档增加敌人类型概率门槛，并区分未检出与真正归档。
- 原因：
  - 试玩反馈认为战斗和日志系统仍缺少商业化确认感，需要优先补强击中、受击、碎片获得和 Boss 读招的即时反馈。
- 影响范围：
  - 攻击命中反馈、受击反馈、Boss 弱点窗口、日志碎片掉落节奏和短 Toast 时长。
- 验证：
  - 已用 Godot 4.6.3 headless 启动 `res://scenes/main.tscn`，无解析错误输出。
- 后续：
  - 仍需要正式音效、粒子、命中特效和 UI 资产配合，才能进一步提升商业完成度。

### UI侧修正：压缩战斗 HUD 与日志页阅读层级

- 负责人：刘秉昂 / Codex
- 改动：
  - 战斗中移除常驻 `P 日志` 提示，只在圣匣、清房和获得日志碎片时提示打开日志。
  - 压缩 `objective_label`、`organ_label`、`relic_label` 和 `dossier_label` 文案，让战斗 HUD 更接近状态标签而不是说明书。
  - 日志页删除顶部长解释，改为短术语说明，并在右侧按标题、来源/类型、正文、判读、主线进度排序。
  - 日志列表增加选中底色条，空日志状态改成“圣匣空槽 / 击杀敌人后自动归档第一份样本”，左侧保留“暂无样本”占位。
- 原因：
  - 当前日志系统已能工作，但 UI 信息密度仍偏高，战斗 HUD 和日志页需要先完成一轮高收益可读性整理。
- 影响范围：
  - 战斗 HUD、日志页覆盖层、日志碎片获得提示、圣匣空状态。
- 验证：
  - 已用 Godot 4.6.3 headless 启动 `res://scenes/main.tscn`，无解析错误输出。
- 后续：
  - 需要正式 UI 资产支持日志页边框、选中条、Toast 背板、分页箭头和空槽图标。

### UI美术：商业化第一轮 HUD 和日志页整理

- 负责人：刘秉昂 / Codex
- 改动：
  - 压缩常驻 HUD 尺寸，减少左上、右上和底部信息框对画面的遮挡。
  - 移除右下角常驻空面板，让日志/采样反馈以短 Toast 方式出现。
  - 降低面板黑底和边框厚重感，改成更轻的半透明暗色 HUD。
  - 将战斗内“采样记录”缩短为归档提示和故事进度，长文本集中到 `P` 日志页阅读。
  - 在日志页补充“吞食胃”和“器官判读”的解释，降低玩家第一次看到系统时的理解成本。
- 原因：
  - 试玩反馈指出当前系统 UI 不够商业化，黑框过多、叠化严重、术语解释不足，整体更像调试界面而不是正式游戏界面。
- 影响范围：
  - 战斗 HUD、目标提示、底部对白条、日志页说明、击杀归档反馈。
- 验证：
  - 已用 Godot 4.6.3 headless 启动 `res://scenes/main.tscn`，无解析错误输出。
- 后续：
  - 下一轮应继续做正式 UI 资产：专用边框、图标、动效、日志页翻页/选中动画，以及战斗 HUD 的响应式缩放。
